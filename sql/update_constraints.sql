    ...
    (widget_type = 'code-ordering'::text)  AND ((payload_question)::jsonb ? 'description'::text) AND ((((payload_question)::jsonb -> 'description'::text))::locale_string IS NOT NULL) AND ((payload_question)::jsonb ? 'lines'::text) AND (jsonb_typeof(((payload_question)::jsonb -> 'lines'::text)) = 'array'::text)) OR (
    (widget_type = 'async-sorter'::text) AND ((payload_question)::jsonb ? 'codeSnippet'::text) AND (jsonb_typeof(((payload_question)::jsonb -> 'codeSnippet'::text)) = 'string'::text) AND ((payload_question)::jsonb ? 'blocks'::text) AND (jsonb_typeof(((payload_question)::jsonb -> 'blocks'::text)) = 'array'::text))
    ))
