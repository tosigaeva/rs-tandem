    ...
    (widget_type = 'async-sorter'::text) AND ((payload_question)::jsonb ? 'code'::text) AND (jsonb_typeof(((payload_question)::jsonb -> 'code'::text)) = 'string'::text) AND ((payload_question)::jsonb ? 'blocks'::text) AND (jsonb_typeof(((payload_question)::jsonb -> 'blocks'::text)) = 'array'::text))
    ))
