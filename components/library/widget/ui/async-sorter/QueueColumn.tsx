...
import { ZoneType } from '@/components/library/widget/ui/async-sorter/DefaultComponent';
...
export function QueueColumn({ id, title, Icon, blocks, onDragStart, onDropBlock }: QueueColumnProperties) {
  return (
    <Card className="flex-1 gap-1">
      ...
      >
        {blocks.length === 0 ? (
          ...
          blocks.map((b) => (
            <div key={b.id} draggable onDragStart={() => onDragStart(b)}>
              <BlockItem code={b.code} label={b.label} />
            </div>
          ))
        )}
      </CardContent>
